...
int main(int argc, char **argv) {
    ...
    cout << " num vertices: " << graph.vertices << endl;
    ...
    int iteration = 0;
    cout<<"active_vertexes: " << active_vertices<<endl;
    while (active_vertices != 0) {
        cout << "active_vertices: " << active_vertices << endl;
        iteration++;
        printf("iteration: %7d: active_vertices: %d\n", iteration, active_vertices);
        std::swap(active_in, active_out);
        active_out->clear();
        graph.hint(parent);
        active_vertices = graph.stream_edges<VertexId>([&](Edge &e) {
 //               cout<<"src: "<< e.source<<", dst: "<< e.target<<endl;
